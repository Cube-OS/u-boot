...
#define UPDATE_COUNT_ENVAR "cubeos_updatecount"
#define DEV_ENVAR          "cubeos_updatedev"
#define PART_ENVAR         "cubeos_updatepart"
#define LOAD_ENVAR         "cubeos_loadaddr"

int update_cubeos_count(void)
{
 ...
 *
 * Input:
 *    cubeosUpdateType upgrade - Indicates whether we're installing a new package as part of an upgrade,
 *        or part of recovery.
 *
 * Returns:
...
int update_cubeos(bool upgrade)
{
    struct mmc *mmc;
    ...
    int ret = CUBEOS_ERR_NO_REBOOT;

    /*
     * Get the name of the update file to load
     */
    file = getenv(CUBEOS_UPDATE_FILE);
    if (file == NULL)
    {
        debug("INFO: %s envar not found\n", CUBEOS_UPDATE_FILE);
        return CUBEOS_ERR_NO_REBOOT;
    }
    ...
    if (upgrade && update_cubeos_count() != 0)
    {
    ...
    if (ret)
    {
        ...
        else
        {
            ...
            if (upgrade)
            {
                ...
                if (strstr(file,"nor") == NULL)
                {
                    char *version = getenv(CUBEOS_CURR_VERSION);
                    setenv(CUBEOS_PREV_VERSION, version);
                    setenv(CUBEOS_CURR_TRIED, "0");
                }
            }
            else
            {
                if (getenv_yesno(CUBEOS_CURR_TRIED) == 1)
                {
                    setenv(CUBEOS_PREV_VERSION, CUBEOS_BASE);
                }
                else
                {
                    setenv(CUBEOS_CURR_TRIED, "1");
                }
            }

            setenv(CUBEOS_CURR_VERSION, file);
            bootcount_store(0);
        }
    }
    else
    ...
    setenv(CUBEOS_UPDATE_FILE, "none");
    setenv(UPDATE_COUNT_ENVAR, "0");
    saveenv();

    return CUBEOS_OK_REBOOT;
}
